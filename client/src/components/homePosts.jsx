        ...
        {postsList && !loading && userList.length !== 0 ? postsList.map((post,index)=>{  
            ...
            <div className="postWidgetCommentSection" style={commentButton?{padding:'5px 10px 15px 10px'}:{display:'none'}}> 
                <div style={{display:'flex'}}>
                    ...
                    <input type='text' onChange={(e)=>setCommentText(e.target.value)} name='comment' value={commentText} placeholder="Add comment ...." style={{marginLeft:'10px',marginRight:'10px',width:'270px',padding:'5px',borderRadius:'20px',backgroundColor:'transparent',borderBottom:'grey',color:'white'}}/> 
                    <button className='sendComment' onClick={()=>handleSubmitComment(post)} style={{height:'fit-content',width:'fit-content',padding:'5px',border:'none',borderRadius:'50%',margin:'auto',display:'flex',justifyContent:'center',backgroundColor:'transparent'}}><LuSendHorizonal size={'1.5em'} style={{margin:'auto',color:'grey'}}/></button>
                </div>
